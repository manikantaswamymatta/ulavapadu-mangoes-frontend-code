...
export default function ProductCard({ product }: { product: ProductCardData }) {
  ...
  const handleAddToCart = async () => {
    ...
    if (hasStockQty && (stockQty ?? 0) <= 0) return;
  ...
  return (
    <div className="product-card">
      <div className="product-image-wrap">
        <img src={resolveCatalogImage(product.image, "/ulavapadumangoes-logo-v2.png")} alt={product.name} loading="lazy" />
      </div>
      ...
      <div className="card-action-row">
        <button
          ...
          disabled={isResolvingProduct || (typeof product.stock_qty === "number" && product.stock_qty <= 0)}
        >
          {typeof product.stock_qty === "number" && product.stock_qty <= 0 ? "Out of stock" : isResolvingProduct ? "Loading..." : addedNotification ? "Added" : "Add"}
        </button>
      </div>
    </div>
  );
